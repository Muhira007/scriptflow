...
import { db } from "@/db";
import { user, generationHistory, appSettings } from "@/db/schema";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (currentUser[0].credits < requiredCredits) {
      return NextResponse.json(
        {
          error: `Kredit tidak cukup. Fitur ini membutuhkan ${requiredCredits} kredit. Silakan Top Up.`,
        },
        { status: 403 },
      );
    }

    // 4b. Determine AI Provider & Fetch API Key
    ...
    if (sourceTab === "image") {
      ...
      try {
        ...
        console.warn(
          "Fetch URL gagal atau diblokir. Melanjutkan dengan slug URL:",
          err,
        );
      }
      ...
      try {
        ...
        const potentialSlug =
          pathParts.find((p) => p.includes("-") && p.length > 5) ||
          pathParts.pop() ||
          "";
        parsedUrlSlug = decodeURIComponent(
          potentialSlug.replace(/-i\.\d+\.\d+.*$/, "").replace(/-/g, " "),
        );
      } catch (e) {}
    ...
    if (!hookOnly) {
      let targetWords = "150-180"; // default 60s
      if (duration === "15") targetWords = "40-50";
      else if (duration === "30") targetWords = "80-100";
      else if (duration === "60") targetWords = "150-180";
      else if (duration === "90") targetWords = "230-260";

      durationGuidance = `- PANJANG NASKAH (SANGAT PENTING): Naskah ini ditargetkan berdurasi ${duration || 60} detik saat dibacakan dengan tempo energik. Agar durasi tercapai dan tidak terlalu cepat habis, kamu WAJIB menulis naskah yang cukup panjang, yaitu sekitar ${targetWords} kata per versinya. JANGAN membuat naskah yang terlalu pendek. Hindari naskah di bawah batas kata tersebut, perbanyak deskripsi yang menarik dan detail!`;
    }
...
PENTING: Output HARUS berupa JSON object dengan struktur berikut (tidak boleh ada teks lain sebelum atau sesudah JSON):
{
  "versionA": ["paragraf 1", "paragraf 2", ...],
  "versionB": ["paragraf 1", "paragraf 2", ...],
...
${durationGuidance}`;
    ...
    // 6. Call AI API with Retry & Fallback
    let response: any;
    ...
    if (aiProvider === "deepseek" && deepseekApiKey) {
      // ===== DEEPSEEK API CALL =====
      const deepseekModels = ["deepseek-v4-flash"];
      const userMessage =
        typeof contents === "string"
          ? contents
          : (contents as Part[])
              .map((c) => (typeof c === "string" ? c : "[Image attached]"))
              .join("\n");
      ...
      for (const modelName of deepseekModels) {
        ...
        for (let attempt = 1; attempt <= attempts; attempt++) {
          try {
            console.log(
              `[Generate] Trying DeepSeek model ${modelName} (Attempt ${attempt}/${attempts})...`,
            );
            ...
            if (!dsResponse.ok) {
              ...
              throw new Error(
                `DeepSeek API error ${dsResponse.status}: ${errBody}`,
              );
            }

            const dsData = await dsResponse.json();
            const content = dsData.choices?.[0]?.message?.content || "";
            inputTokens = dsData.usage?.prompt_tokens || 0;
            outputTokens = dsData.usage?.completion_tokens || 0;

            if (content) {
              response = { text: content };
              console.log(`[Generate] Success using DeepSeek ${modelName}`);
              break;
            }
          } catch (err: any) {
            lastError = err;
            console.warn(
              `[Generate] DeepSeek ${modelName} attempt ${attempt} failed:`,
              err.message || err,
            );
            if (attempt < attempts) {
      ...
      for (const modelName of modelsToTry) {
        ...
        for (let attempt = 1; attempt <= attempts; attempt++) {
          try {
            console.log(
              `[Generate] Trying Gemini model ${modelName} (Attempt ${attempt}/${attempts})...`,
            );
                ...
                responseSchema: {
                  ...
                  properties: {
                    versionA: {
                      ...
                      items: { type: "STRING" },
                      description:
                        "Kumpulan paragraf naskah narasi versi 1 (Hard-Selling/To The Point). 1 item array = 1 paragraf.",
                    },
                    versionB: {
                      type: "ARRAY",
                      items: { type: "STRING" },
                      description:
                        "Kumpulan paragraf naskah narasi versi 2 (Storytelling/Emosional). 1 item array = 1 paragraf.",
                    },
                    caption: {
                      type: "ARRAY",
                      items: { type: "STRING" },
            ...
            if (geminiResponse && geminiResponse.text) {
              ...
              console.log(`[Generate] Success using Gemini ${modelName}`);
              break;
            }
          } catch (err: any) {
            lastError = err;
            console.warn(
              `[Generate] Gemini ${modelName} attempt ${attempt} failed:`,
              err.message || err,
            );
            if (attempt < attempts) {
              await new Promise((resolve) =>
